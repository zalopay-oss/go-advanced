# Summary

* [Giới thiệu](README.md)
* [Chương 1: Nền tảng ngôn ngữ Go](ch1-basic/README.md)
    * [1.1 Nguồn gốc của ngôn ngữ Go](ch1-basic/ch1-01-genesis.md)
    * [1.2 Sự tiến hóa của chương trình "Hello World"](ch1-basic/ch1-02-hello-revolution.md)
    * [1.3 Array, strings và slices](ch1-basic/ch1-03-array-string-and-slice.md)
    * [1.4 Functions, Methods và Interfaces](ch1-basic/ch1-04-func-method-interface.md)
    * [1.5 Khái niệm xử lý đồng thời và song song](ch1-basic/ch1-05-concurrency-parallelism.md)
    * [1.6 Mô hình thực thi đồng thời](ch1-basic/ch1-06-common-concurrency-mode.md)
    * [1.7 Error và Exceptions](ch1-basic/ch1-07-error-and-panic.md)
* [Chương 2: Lập trình CGO](ch2-cgo/README.md)
    * [2.1 Quick Start](ch2-cgo/ch2-01-quick-start.md)
    * [2.2 CGO Foundation](ch2-cgo/ch2-02-CGO-foundation.md)
    * [2.3 Chuyển đổi kiểu](ch2-cgo/ch2-03-type-conversion.md)
    * [2.4 Lời gọi hàm](ch2-cgo/ch2-04-function-call.md)
    * [2.5 Cơ chế bên trong CGO](ch2-cgo/ch2-05-internal-mechanisms.md)
    * [2.9 Thư viện tĩnh và động](ch2-cgo/ch2-09-static-shared-lib.md)
    * [2.10 Biên dịch và liên kết các tham số](ch2-cgo/ch2-10-link.md)
    * [2.11 Lời nói thêm](ch2-cgo/ch2-11-ext.md)
* [Chương 3: Remote Procedure Call](ch3-rpc/README.md)
    * [3.1 Giới thiệu về RPC](ch3-rpc/ch3-01-rpc-go.md)
    * [3.2 Protobuf](ch3-rpc/ch3-02-pb-intro.md)
    * [3.3 Làm quen với gRPC](ch3-rpc/ch3-03-grpc.md)
    * [3.4 Một số vấn đề khác của gRPC](ch3-rpc/ch3-04-grpc-expand.md)
    * [3.5 gRPC và Protobuf extensions](ch3-rpc/ch3-05-grpc-ext.md)
    * [3.6 Công cụ grpcurl](ch3-rpc/ch3-06-grpc-curl.mdd)
    * [3.7 Lời nói thêm](ch3-rpc/ch3-07-supplementary-notes.md)
* [Chương 4: Go và Web](ch4-web/README.md)
    * [4.1 Giới thiệu chương trình Web](ch4-web/ch4-01-intro.md)
    * [4.2 Routing trong Web](ch4-web/ch4-02-router.md)
    * [4.3 Middleware](ch4-web/ch4-03-middleware.md)
    * [4.4 Kiểm tra tính hợp lệ của request](ch4-web/ch4-04-request-verification.md)
    * [4.5 Làm việc với Database](ch4-web/ch4-05-database.md)
    * [4.6 Giới hạn lưu lượng Service](ch4-web/ch4-06-service-flow-limitation.md)
    * [4.7 Mô hình của các dự án web](ch4-web/ch4-07-layout-of-web-project.md)
    * [4.8 Lời nói thêm](ch4-web/ch4-08-ext.md)
* [Chương 5: Hệ thống phân tán](ch5-distributed-system/README.md)
    * [5.1 Distributed id generator](ch5-distributed-system/ch5-01-dist-id.md)
    * [5.2 Lock phân tán](ch5-distributed-system/ch5-02-lock.md)
    * [5.3 Hệ thống tác vụ có trì hoãn](ch5-distributed-system/ch5-03-delay-job.md)
    * [5.4 Cân bằng tải](ch5-distributed-system/ch5-04-load-balance.md)
    * [5.5 Quản lý cấu hình trong hệ thống phân tán](ch5-distributed-system/ch5-05-config.md)
    * [5.6 Trình thu thập thông tin phân tán](ch5-distributed-system/ch5-06-crawler.md)
    * [5.7 Lời nói thêm](ch5-distributed-system/ch5-07-ext.md)
* [Chương 6: Go Best Practices](ch6-best-practice/README.md)

