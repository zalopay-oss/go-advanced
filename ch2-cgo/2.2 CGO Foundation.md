...
Để sử dụng CGO, ta cần cài trình dịch C/C++ và chắc chắn rằng `CGO_ENABLE` được bật.
...
Bây giờ có thể ta muốn sử dụng function này trong các gói Go khác:
```go
package main
...
import "C"
```
Trong dòng ghi chú `import "C"` phía trước các lệnh. Bạn có thể đặt các tham số cho quá trình biên dịch và quá trình liên kết thông qua các lệnh `#cgo`. ác tham số của qus trình biên dịch chủ yếu được sử dụng để xác định các macro liên quan và đường dẫn truy xuất file header đã chỉ định. Các tham số của quá trình liên kết chủ yếu là để xác định đường dẫn truy xuất file thư viện và file thư viện sẽ được liên kết.
...
Đối với người dùng sử dụng C và C++ trong môi trường cgo, có thể có ba tùy chọn biên dịch khác nhau: CFLAGS cho các tùy chọn biên dịch theo ngôn ngữ C, CXXFLAGS cho các tùy chọn biên dịch cụ thể C++ và CPPFLAGS cho các biên dịch C và C++. Tùy chọn. Tuy nhiên, trong giai đoạn liên kết, các tùy chọn liên kết C và C++ là chung, do đó không còn sự khác biệt giữa C và C++ tại thời điểm này và các file mục tiêu của chúng cùng loại.
...
Bằng cách này, chúng ta có thể sử dụng các kỹ thuật thường được sử dụng trong C để xử lý mã khác biệt giữa các nền tảng khác nhau.

2.2.3 Biên dịch có điều kiện với build tag
---
...
Trong đó `linux,386` và `linux` và `386` đại diện bởi một liên kết, dấu phẩy nghĩa là `;` và `linux,386` và `darwin,!cgo` giữa OR đại diện bởi phân chia trống nghĩa.???
