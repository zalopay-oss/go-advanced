...
Bộ thực thi Go có một bộ concurrency cho riêng nó, nó dùng một số kĩ thuật để ghép kênh M Goroutines trên N thread của hệ thống. Cơ chế định thời goroutine tương tự với kernel, nhưng bộ định thời  chỉ tập trung vào việc Goroutines trên các chương trình Go riêng biệt. Goroutine sử dụng cơ chế `semi-preemptive cooperative scheduling`, nó có thể gây ra việc định thời (scheduler) khi mà chương trình goroutines hiện tại bị block, Cùng thời điểm đó. nó sẽ switch sang user mode. Bộ định thời chỉ lưu trữ những thanh ghi cần thiết cho một vài hàm đặc biệt, và chi phí chuyển ngữ cảnh của các goroutines sẽ thấp hơn nhiều so với việc chuyển ngữ cảnh của thread hệ thống. Bộ runtime có một biến là `runtime.GOMAXPROCS` nó sẽ điều khiển số lượng system thread hiện thời chạy trên cơ chế non-blocking Goroutine thông thường.

Bắt đầu một goroutine trong go không chỉ là gọi một hàm, mà là kèm theo chi phí của việc định thời giữa các Goroutines. Những đặc điểm đó có sự ảnh hưởng lớn đến sự phổ biến và phát triển của lập trình concurrency.
